...
class Search(Handler):
    ...
    def update_prompt(self) -> None:
        base_prompt = "~>" if self.mode == "regex" else "=>"
        if self.match_count > 0:
            self.prompt = f"[{self.current_match}/{self.match_count}] {base_prompt} "
        else:
            self.prompt = f"[0/0] {base_prompt} "
    ...
    def count_matches(self) -> int:
        """Count total matches in all windows."""
        total = 0
        text = self.line_edit.current_input
        if not text or not self.window_ids:
            return 0
        try:
            pattern = text if self.mode == "regex" else re.escape(text)
            flags = 0 if text.islower() else re.IGNORECASE
        ...
        if text:
            ...
            for match_arg in self.match_args():
                try:
                    ...
                    self.remove_mark()
            self.match_count = self.count_matches()
            self.current_match = 1 if self.match_count > 0 else 0
        else:
            self.remove_mark()
            self.match_count = 0
            self.current_match = 0
